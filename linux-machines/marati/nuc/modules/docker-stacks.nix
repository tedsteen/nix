...
with lib;
{
    ...
    stacks = mkOption {
      type = types.attrsOf (types.submodule ({ ... }: {
        ...
        options.path = mkOption {
          type        = types.path;
          description = "Path to the stack.";
        };
        options.manageScriptName = mkOption {
          type        = types.str;
          description = "Name of the manage script.";
          default     = "manage.sh";
        };
  ...
  in {
    ...
    environment.systemPackages = let
      mkDockerStack = name: path: scriptName: pkgs.stdenv.mkDerivation {
        ...
        installPhase = ''
          mkdir -p $out/${name}
          cp -r . $out/${name}
          patchShebangs $out/${name}/${scriptName}
          mkdir -p $out/bin
          ln -s $out/${name}/${scriptName} $out/bin/docker-manage-stack-${name}
        '';
      };
    in mapAttrsToList (name: sCfg: mkDockerStack name sCfg.path sCfg.manageScriptName) cfg.stacks ++ [
